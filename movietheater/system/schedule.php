...
if(isLoggedIn()) {
    ...
    if(isAdmin($user) || isEmployee($user)) {
        $__pageError = getTransfer("scheduleError");
        $__pageSuccess = getTransfer("scheduleSuccess");
?>
            ...
            <div class="modal-dialog modal-confirm">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Do you really want to delete a Schedule? This process cannot be undone.</p>
                    </div>
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h1 class="page-header">Movie Schedules</h1>
                        </div>
                        <!-- /.col-lg-12 -->
                    </div>
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    Showing All Movie Schedules From System
                                    <?php if(isEmployee($user)) { ?>
                                        <a href="scheduleadd.php" class="btn btn-primary btn-xs pull-right">Add New</a>
                                    <?php } ?>
                                </div>
                                <!-- /.panel-heading -->
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                            ...
                                            <tbody>
                                                <?php $schedules = getSchedules(); ?>
                                                <?php if(!empty($schedules)) : ?>
                                                <?php foreach ($schedules as $__schedule) : ?>
                                                <?php $__movie = getMovieById($__schedule['movie_id']); ?>
                                                <tr>
                                                    <td align="center"><img src="<?= getPoster($__movie['poster']) ?>" height="60px" width="60px"></td>
                                                    <td><?= htmlspecialchars($__movie['name']) ?></td>
                                                    <td><?= htmlspecialchars($__schedule['hall']) ?></td>
                                                    <td><?= ($__schedule['seat_column']*$__schedule['seat_row']) . " (" . $__schedule['seat_column']."x".$__schedule['seat_row']. ")" ?></td>
                                                    <td><?= __formatDate($__schedule['start'], "j/n/Y ( g:i a"). " to ". __formatDate($__schedule['end'], "g:i a )") ?></td>
                                                    <td><?= __preiodStatus($__schedule['start'], $__schedule['end']) ?></td>
                                                    <td>
                                                        <a href="scheduleedit.php?schedule=<?= htmlspecialchars($__schedule['id']) ?>" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a>&nbsp; &nbsp;
                                                        <a href="scheduledelete.php?schedule=<?= htmlspecialchars($__schedule['id']) ?>" class="btn btn-danger btn-xs remove-confirm"><i class="fa fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                                <?php endforeach; ?>
                                                <?php else : ?>
